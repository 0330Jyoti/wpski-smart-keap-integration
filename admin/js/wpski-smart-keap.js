 (function($) { 
	...
	var wp_module_name, Keap_module_name;
	...
	jQuery(document).on("change", "select[name=Keap_module]", function(){
		...
		Keap_module_name = jQuery(this).val();
		...
		jQuery.ajax({
	        ...
	        data: {
	            ...
	            'Keap_module_name': Keap_module_name

	        },

	        success: function( response ) {

	            jQuery("select[name=Keap_field]").empty().append(response);
